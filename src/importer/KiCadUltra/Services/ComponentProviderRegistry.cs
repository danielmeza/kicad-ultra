...
public class ComponentProviderRegistry : IComponentProviderRegistry
{
    ...
    {
        List<IComponentProvider> enabled = EnabledProvidersList;
        if (_configService != null && !string.IsNullOrEmpty(_configService.DefaultProviderId))
        {
            IComponentProvider? def = enabled.FirstOrDefault(p => string.Equals(p.Id, _configService.DefaultProviderId, StringComparison.OrdinalIgnoreCase));
