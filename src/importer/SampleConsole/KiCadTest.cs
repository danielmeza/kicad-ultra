...
internal class KiCadTest
{
    ...
    private static void TestSymbolParsing()
    {
        ...
        var symbolFilePath = @"c:\Users\danie\AppData\Roaming\UltralibrarianKicad\ul_STUSB4500LQTR\KiCADv6\2025-05-01_22-48-51.kicad_sym";
        ...
        if (File.Exists(symbolFilePath))
        {
            try
            {
                ...
                SExpression expression = parser.ParseFile(symbolFilePath);
                ...
                {
                    KiCadSymbol symbol = library.Symbols[0];
                    Console.WriteLine($"\nSymbol Id: {symbol.Id}");
                    Console.WriteLine("Properties:");
                    foreach (KiCadProperty property in symbol.Properties)
                    {
                    ...
                    var outputPath = Path.Combine(Path.GetTempPath(), "test_symbol.kicad_sym");
    ...
    private static void TestCreateSymbol()
    {
        ...
        try
        {
            ...
            // Set properties
            _ = symbol.AddProperty("Manufacturer", "Test Manufacturer");
            _ = symbol.AddProperty("Manufacturer_Part_Number", "TEST123");
    ...
    private static void TestCreateFootprint()
    {
        ...
        try
        {
            ...
            // Save the footprint
            var outputPath = Path.Combine(Path.GetTempPath(), "test_created_footprint.kicad_mod");
