...
public class UltraLibrarianImporter
{
    ...
    public async Task<ImportResult> ImportComponentAsync(string zipFilePath, ImportType importType)
    {
        ...
        var tempDir = Path.Combine(Path.GetTempPath(), $"UltraLibrarian_Import_{Guid.NewGuid()}");
    ...
    private void GetProjectPath()
    {
        ...
        {
            var targetDir = Path.GetDirectoryName(_projectPath);
    ...
    private async Task<bool> ImportSymbolsAsync(string extractDir)
    {
        try
        {
            ...
            var libraryBaseName = !string.IsNullOrEmpty(_options.LibraryName)
            ...
            var symbolLibPath = string.Empty;
            ...
            foreach (var symbolFile in symbolFiles)
            {
                try
                {
                    var fileName = Path.GetFileName(symbolFile);
    ...
    private async Task<bool> ImportFootprintsAsync(string extractDir)
    {
        try
        {
            ...
            var footprintLibPath = string.Empty;
            ...
            foreach (var prettyDir in prettyDirs)
            {
                try
                {
                    var dirName = Path.GetFileName(prettyDir);
                    ...
                    foreach (var footprintFile in footprintFiles)
                    {
                        try
                        {
                            var fileName = Path.GetFileName(footprintFile);
                            _logger.LogDebug($"Processing footprint: {fileName}");

                            // Load the footprint
                            var sourceLib = KiCadFootprintLibrary.Load(footprintFile);

                            if (sourceLib.Footprints.Count <= 0)
                            {
                                continue;
                            }
                            foreach (KiCadFootprint footprint in sourceLib.Footprints)
                            {

                                // Rename footprint to avoid conflicts
                                var newFootprintId = $"UL_{footprint.Id}";
                                var newFootprint = new KiCadFootprint(newFootprintId);

                                // Copy properties from original footprint
                                // Pads
                                foreach (KiCadPad pad in footprint.Pads)
                                {
                                    newFootprint.Pads.Add(pad);
                                }

                                // Text items
                                foreach (KiCadFpText text in footprint.TextItems)
                                {
                                    newFootprint.TextItems.Add(text);
                                }

                                // Lines
                                foreach (KiCadFpLine line in footprint.Lines)
                                {
                                    newFootprint.Lines.Add(line);
                                }

                                // Circles
                                foreach (KiCadFpCircle circle in footprint.Circles)
                                {
                                    newFootprint.Circles.Add(circle);
                                }

                                // Arcs
                                foreach (KiCadFpArc arc in footprint.Arcs)
                                {
                                    newFootprint.Arcs.Add(arc);
                                }

                                // Polygons
                                foreach (KiCadFpPoly poly in footprint.Polygons)
                                {
                                    newFootprint.Polygons.Add(poly);
                                }

                                // 3D Models
                                foreach (KiCadModel model in footprint.Models)
                                {
                                    newFootprint.Models.Add(model);
                                }

                                // Save the footprint to the target library
                                var destPath = Path.Combine(footprintLibPath, $"{newFootprintId}.kicad_mod");
            ...
            foreach (var footprintFile in individualFootprintFiles)
            {
                try
                {
                    ...
                    if (sourceLib.Footprints.Count > 0)
                    {
                        KiCadFootprint footprint = sourceLib.Footprints[0];
    ...
    private async Task AddSymbolLibraryToTable(string libraryPath, string libraryName)
    {
        ...
        RunActionResponse result = await _kicad.RunAction($"common.Control.addLibrary");

        if (result.Status == RunActionStatus.RasOk)
        {
            _logger.LogInformation("Symbol library added to library table: {Name} ({Path})", libraryName, libraryPath);
        }
    ...
    private async Task<string> GetSymbolLibraryPath()
    {
        ...
        if (!string.IsNullOrEmpty(_projectPath))
        {
            var projectDir = DirectoryOf(_projectPath);
            var symLibTable = Path.Combine(projectDir, "sym-lib-table");
    ...
    private async Task<string> GetFootprintLibraryPath()
    {
        ...
        {
            var projectDir = DirectoryOf(_projectPath);
            var fpLibTable = Path.Combine(projectDir, "fp-lib-table");
    ...
    private async Task<string> Get3DModelPath()
    {
        ...
        {
            var projectDir = DirectoryOf(_projectPath);
            var modelDir = Path.Combine(projectDir, "3d_models");
            _ = Directory.CreateDirectory(modelDir);
            return modelDir;
        }
